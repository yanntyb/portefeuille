...
it('sert un worker inerte tant que le runtime n\'est pas compilé', function () {
    File::delete(public_path('sw-runtime.js'));
...
- [ ] **Step 3: Écrire l'assembleur PHP**
...
- [ ] **Step 4: Brancher la vue et la route**
...
- [ ] **Step 5: Écrire le runtime du worker**
...
- [ ] **Step 6: Écrire le second config Vite**
...
- [ ] **Step 7: Enchaîner les deux builds**
...
- [ ] **Step 8: Compiler et vérifier la sortie**
...
- [ ] **Step 10: Ignorer la sortie de build**
...
```bash
vendor/bin/pint --dirty --format agent
git add app/Shared/Pwa/ServiceWorkerScript.php resources/js/pwa/sw.ts vite.sw.config.ts resources/views/pwa/sw.blade.php routes/pwa.php package.json tests/Feature/PwaRoutesTest.php .gitignore
...
Créer `tests/Browser/PwaTest.php` :
...
Run: `bun run build && bun run test:pwa`
Expected: `Parcours hors-ligne : OK`, sortie sans erreur.

Si le script échoue sur l'attente du worker, vérifier que `public/sw-runtime.js` existe : sans lui, la route sert le worker inerte, qui se désenregistre.
