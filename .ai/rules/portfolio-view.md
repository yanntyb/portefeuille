...
Le volet portefeuille — les deux contrôleurs et `BuildPortfolioViewSnapshot` — ne touche aucune action ni Data de Portfolio, Valuation ou Income. Tout passe par `Ports/` : `PortfolioOverviewPort`, `ValuationPort`, `SectorBreakdownPort`, `IncomePort`, plus les trois plus anciens. Un port par contexte voisin, pas par action : `ValuationPort` porte les quatre lectures de valorisation, aux deux échelles.
