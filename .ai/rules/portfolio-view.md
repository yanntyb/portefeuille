...
Le périmètre passe par `Market\Datas\HoldingScope`, jamais par un `AssetClass` : `overviewFor`, `classBreakdownFor`, `analysisFor`, `performancesFor`, `evolutionFor`, `breakdownFor`, `transactionsForScope`. Aucune exception. Enveloppe et exposition lisent la même `evolutionFor()`, donc la même `EvolutionSection` avec sa bascule valeur/détail ; le total s'y somme côté page, la valorisation ne portant pas les espèces. `BasketAnalysis` est la seule composition d'analyse : ne pas en écrire une seconde pour une nouvelle découpe — lui passer un périmètre suffit.
