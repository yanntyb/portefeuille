...
Depuis que les trois Datas jumelles d'opération (`MarketView\TransactionLineData`, `ClassTransactionLineData`, `Wealth\WealthTransactionLineData`) exposent `id`, un auto-incrément entre dans le JSON de `tests/Feature/SnapshotInvariantTest.php`. Le hash reste stable — SQLite en mémoire sous `RefreshDatabase` remet la séquence à zéro à chaque test — mais **réordonner les créations de `Transaction::factory()`** dans `portfolioFixture()`, `cryptoFixture()` ou `seedSnapshotFixture()` déplacera le hash pour une raison qui n'a rien à voir avec la forme du JSON. Ajouter une transaction en fin de fixture est sans effet sur les précédentes ; en insérer une au milieu décale tout ce qui suit. Ne pas neutraliser `id` dans `normalizeSnapshotBody()` : c'est désormais une vraie clé de forme, le filet doit la voir.
