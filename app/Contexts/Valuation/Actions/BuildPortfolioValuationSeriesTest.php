...
it('windows and aggregates the portfolio series by range and granularity', function () {
    $user = User::factory()->create();
    $wallet = Wallet::factory()->for($user)->create();
    $asset = Instrument::factory()->create();
    ...
    $monthly = app(BuildPortfolioValuationSeries::class)($user->id, ValuationRange::Max, ValuationGranularity::Month);
    expect($monthly->labels)->toBe(['2026-01-31', '2026-02-28', '2026-03-31']);

    $lastMonth = app(BuildPortfolioValuationSeries::class)($user->id, ValuationRange::OneMonth, ValuationGranularity::Month);
    expect($lastMonth->labels)->toBe(['2026-02-28', '2026-03-31']);
});
