...
class LaravelSeriesCache implements SeriesCachePort
{
     ...
     *
     * Chaque type est compté à part : un changement de type ne bouge ni le nombre de lignes, ni
     * aucune des sommes ci-dessus (même quantité, même prix, même actif, même montant), et
     * `updated_at` ne descend pas sous la seconde. Compter les seules ventes laissait passer un
     * versement basculé en retrait — même montant, même date, même enveloppe, et pourtant le
     * signe du mouvement s'inverse. Sans ces comptes, la série périmée serait servie jusqu'à la
     * prochaine modification tombant dans une autre seconde.
     *
            ...
            (string) json_encode(Transaction::query()
                ...
                ->tap(function ($query): void {
                    foreach (TransactionType::cases() as $type) {
                        $query->selectRaw(
                            sprintf('coalesce(sum(case when type = ? then 1 else 0 end), 0) as count_%s', $type->value),
                            [$type->value],
                        );
                    }
                })
