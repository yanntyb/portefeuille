...
it('decodes a successful envelope into a PythonResult', function () {
    ...
    $runner = new ProcessPythonRunner($factory, '/usr/bin/python', storage_path('python/scripts'), 30);

    $result = $runner->run('fetch_prices.py', ['ticker' => 'AAPL']);
...
it('returns an error envelope without throwing on a successful process', function () {
    ...
    $result = $runner->run('fetch_prices.py');
...
it('throws when the script file is missing', function () {
    $runner = new ProcessPythonRunner(Process::fake(), '/usr/bin/python', storage_path('python/scripts'), 30);

    $runner->run('does_not_exist.py');
...
it('runs the configured binary against the script path', function () {
    ...
    $runner = new ProcessPythonRunner($factory, '/custom/python', storage_path('python/scripts'), 30);

    $runner->run('fetch_prices.py', ['ticker' => 'MSFT']);

    Process::assertRan(fn ($process) => str_contains($process->command, '/custom/python')
        && str_contains($process->command, 'fetch_prices.py'));
});
